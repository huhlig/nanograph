...
use nanograph_core::object::{ClusterMetadata, StorageEngineType};
use nanograph_core::{
    object::{NodeId, ShardId, ShardMetadata, ShardStatus},
    types::Timestamp,
};
use std::sync::Arc;
...
pub struct SystemShardRaftGroup {
    /// Local node ID
...
mod tests {
    ...
    async fn test_metadata_group() {
        let group = SystemShardRaftGroup::new(NodeId::new(1));
