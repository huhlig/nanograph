...
fn bench_large_sequential_write() {
    ...
    for i in 0..num_keys {
        ...
        let value = format!("value_{:010}_with_some_additional_data_to_make_it_realistic", i);
    ...
    println!("  Throughput: {:.2} ops/sec", num_keys as f64 / duration.as_secs_f64());
    println!("  Avg latency: {:.2} µs/op", duration.as_micros() as f64 / num_keys as f64);
}
...
fn bench_large_value_writes() {
    ...
    println!("  Total time: {:?}", duration);
    println!("  Throughput: {:.2} ops/sec", num_keys as f64 / duration.as_secs_f64());
    println!("  Data written: {:.2} MB", total_bytes as f64 / (1024.0 * 1024.0));
    println!("  Write bandwidth: {:.2} MB/s", 
...
fn bench_mixed_workload() {
    ...
    let duration = start.elapsed();
    println!("  Total time: {:?}", duration);
    println!("  Total ops: {} ({} reads, {} writes)", num_ops, reads, writes);
    println!("  Throughput: {:.2} ops/sec", num_ops as f64 / duration.as_secs_f64());
    println!("  Avg latency: {:.2} µs/op", duration.as_micros() as f64 / num_ops as f64);
}
