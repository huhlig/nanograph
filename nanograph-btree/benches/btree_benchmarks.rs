...
async fn create_populated_store(count: usize) -> (BTreeKeyValueStore, nanograph_kvt::KeyValueTableId) {
    let store = BTreeKeyValueStore::default();
    let table = store.create_table("bench").await.unwrap();
    let kvs = generate_sequential_kvs(count);
    ...
    for batch_size in [10, 100, 1_000].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(batch_size), batch_size, |b, &batch_size| {
            ...
            b.to_async(&rt).iter(|| async {
                let store = BTreeKeyValueStore::default();
                let table = store.create_table("bench").await.unwrap();
                store.batch_put(table, black_box(&pairs)).await.unwrap();
            });
        });
    }
    ...
    group.bench_function("90_read_10_write", |b| {
        ...
        b.to_async(&rt).iter(|| async {
            ...
            for i in 0..10 {
                ...
                store.put(table, black_box(&key), black_box(&value)).await.unwrap();
            }
        });
    });
...
fn bench_tree_height_impact(c: &mut Criterion) {
    ...
    for max_keys in [8, 32, 128, 512].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("max_keys", max_keys),
            max_keys,
            |b, _| {
                b.iter(|| {
                    let tree = BPlusTree::new(config.clone());
