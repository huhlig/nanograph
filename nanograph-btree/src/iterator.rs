    ...
    fn find_forward_start(
        ...
        match start_bound {
            ...
            Bound::Included(key) | Bound::Excluded(key) => {
                ...
                let leaf_id = tree.find_leaf(
                    *tree.root_id().read().unwrap(),
                    key,
                )?;
                let entries = tree.get_leaf_entries(leaf_id)?;
    ...
    fn find_reverse_start(
        ...
        match end_bound {
            Bound::Unbounded => {
                ...
                let entries = tree.get_leaf_entries(leaf_id)?;
                let start_index = if entries.is_empty() { 0 } else { entries.len() - 1 };
                Ok((Some(leaf_id), entries, start_index))
            }
            Bound::Included(key) | Bound::Excluded(key) => {
            ...
            if need_next_leaf {
                let next_leaf = if self.reverse {
                    match self.tree.get_prev_leaf(self.current_leaf.unwrap()) {
                        Ok(leaf) => leaf,
                        Err(e) => return Poll::Ready(Some(Err(nanograph_kvt::KeyValueError::from(e)))),
                    }
                } else {
...
impl KeyValueIterator for BPlusTreeIterator {
    fn seek(&mut self, key: &[u8]) -> nanograph_kvt::KeyValueResult<()> {
        ...
        let leaf_id = self.tree.find_leaf(root_id, key).map_err(|e| nanograph_kvt::KeyValueError::from(e))?;
        
        // Get leaf entries through public method
        let entries = self.tree.get_leaf_entries(leaf_id).map_err(|e| nanograph_kvt::KeyValueError::from(e))?;
        
        self.current_leaf = Some(leaf_id);
        self.current_entries = entries;
        
        // Find the position of the key or where it would be inserted
        let idx = self.current_entries.binary_search_by(|(k, _)| k.as_slice().cmp(key))
            .unwrap_or_else(|idx| idx);
