//! Transaction usage example for Nanograph B+Tree
//!
...
use nanograph_kvt::{KeyValueShardStore, ShardIndex, TableId, Transaction};
use std::sync::Arc;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== Nanograph B+Tree Transaction Example ===\n");

    // Create the store (transaction manager is initialized by default)
    let store = Arc::new(BTreeKeyValueStore::default());
    println!("✓ Created B+Tree KeyValueStore with transaction support");
